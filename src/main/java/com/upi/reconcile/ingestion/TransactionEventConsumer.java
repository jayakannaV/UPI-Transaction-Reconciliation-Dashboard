 ...
 * <li><b>DB fallback</b> — query {@code webhook_events} table if Redis
 * misses</li>
 * <li><b>If duplicate</b> — log to {@code webhook_events} with
 ...
 *
 * <p>
 * <b>Manual ack</b>: offset is committed only after the DB write succeeds
 * (via {@link Acknowledgment#acknowledge()}). A consumer crash mid-processing
 * triggers Kafka redelivery → idempotency check prevents duplicate
 * transactions.
 */
@Slf4j
@Component
...
public class TransactionEventConsumer {
        ...
        @KafkaListener(topics = TransactionEventProducer.WEBHOOK_TOPIC, groupId = "${spring.kafka.consumer.group-id}", containerFactory = "kafkaListenerContainerFactory")
