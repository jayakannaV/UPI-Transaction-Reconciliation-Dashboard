...
public class BankController {
        ...
        public ResponseEntity<List<BankScorecardDto>> getBankScorecard() {
                ...
                // Tenant isolation: only include the authenticated merchant's transactions
                UUID merchantId = MerchantContextHolder.currentMerchantId();
                List<Transaction> merchantTransactions = allTransactions.stream()
                                .filter(txn -> txn.getMerchantOwner() != null
                                                && merchantId.equals(txn.getMerchantOwner().getMerchantId()))
                                .toList();
                ...
                for (Transaction txn : merchantTransactions) {
                        if (txn.getRemitterBank() == null)
                                continue;
                ...
                List<BankScorecardDto> scorecards = banks.stream()
                                .map(bank -> {
                                        ...
                                        Map<TransactionState, Long> stateCounts = bankStateCounts.getOrDefault(bankId,
                                        ...
                                        long failures = stateCounts.getOrDefault(TransactionState.TECHNICAL_DECLINED,
                                                        0L)
                                                        + stateCounts.getOrDefault(TransactionState.DEEMED_APPROVED,
                                                                        0L);
                                        ...
                                        return BankScorecardDto.builder()
                                                        ...
                                                        .historicalDeemedApprovedRate(
                                                                        bank.getHistoricalDeemedApprovedRate())
